    ...
    async create(fields) {
        try {
            ...
            const foreignKeys = fields
                ...
                .map(
                    (field) =>
                        `, FOREIGN KEY ("${field.key}") REFERENCES "${field.foreing.table}"("${field.foreing.column}")`
                )
                .join(' ');
        ...
        try {
            ...
            for (const field of fields) {
                ...
                if (!currentFields[key]) {
                    ...
                    if (foreing) {
                        alterQuery += `, ADD CONSTRAINT fk_${key} FOREIGN KEY (${key}) REFERENCES ${foreing.table}(${foreing.column})`;
                    }
                ...
                if (currentFields[key]) {
                    ...
                    if (existingField.type !== fullType || existingField.defaultValue !== defaultValue ||
                        ...
                        (options && options.includes('primary') && existingField.key !== 'PRI')) {
                            ...
                            let modifyQuery = existingField.type !== fullType ? `ALTER TABLE ${this.tableName} ALTER COLUMN ${key} TYPE ${fullType};\n ALTER TABLE ${this.tableName} ALTER COLUMN ${key}` : `ALTER TABLE ${this.tableName} ALTER COLUMN ${key}`;
                            ...
                            if (existingField.defaultValue !== defaultValue) {
                                ...
                                if (foreing) {
                                    modifyQuery += `, ADD CONSTRAINT fk_${key} FOREIGN KEY (${key}) REFERENCES ${foreing.table}(${foreing.column})`;
                                }
